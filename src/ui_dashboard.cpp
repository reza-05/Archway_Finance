...
static void RenderCharts(AppState& state) {
    if (state.accounts.empty()) {
        ImGui::Text("No account data available. Add an account/wallet to display analytics.");
        return;
    }

    // Top Section: Side-by-side plots (Account Allocation Bar and Cash Flow Bar)
    ImGui::Columns(2, "PlotsLayout", false);
    
    ImGui::TextDisabled("WALLET VALUE ALLOCATION (CURRENT BALANCE)");
    ImGui::Spacing();
    if (ImPlot::BeginPlot("Account Allocation", ImVec2(-1, 230), ImPlotFlags_NoMouseText)) {
        ImPlot::SetupAxes("Accounts", "Balance (BDT)", ImPlotAxisFlags_AutoFit, ImPlotAxisFlags_AutoFit);
        
        std::vector<const char*> labels;
        std::vector<double> ticks;
        std::vector<double> balances;
        for (size_t i = 0; i < state.accounts.size(); ++i) {
            labels.push_back(state.accounts[i].name.c_str());
            ticks.push_back((double)i);
            balances.push_back(state.accounts[i].current_balance);
        }
        ImPlot::SetupAxisTicks(ImAxis_X1, ticks.data(), (int)ticks.size(), labels.data());
        
        ImPlot::SetNextFillStyle(ImVec4(0.41f, 0.34f, 0.74f, 0.80f));
        ImPlot::PlotBars("Balance", ticks.data(), balances.data(), (int)balances.size(), 0.4);
        ImPlot::EndPlot();
    }
    
    ImGui::NextColumn();
    
    ImGui::TextDisabled("CASH FLOW COMPARISON");
    ImGui::Spacing();
    if (ImPlot::BeginPlot("Cash Flow", ImVec2(-1, 230), ImPlotFlags_NoMouseText)) {
        ImPlot::SetupAxes("Type", "Amount (BDT)", ImPlotAxisFlags_AutoFit, ImPlotAxisFlags_AutoFit);
        const char* flow_labels[] = {"Total Income", "Total Expenses"};
        double flow_ticks[] = {0.0, 1.0};
        double flow_values[] = {state.total_income, state.total_expense};
        ImPlot::SetupAxisTicks(ImAxis_X1, flow_ticks, 2, flow_labels);
        
        ImPlot::SetNextFillStyle(ImVec4(0.12f, 0.73f, 0.38f, 0.80f));
        ImPlot::PlotBars("Income", &flow_ticks[0], &flow_values[0], 1, 0.3);
        
        ImPlot::SetNextFillStyle(ImVec4(0.89f, 0.22f, 0.22f, 0.80f));
        ImPlot::PlotBars("Expense", &flow_ticks[1], &flow_values[1], 1, 0.3);
    ...
    // Bottom Section: Full Width Balance Trend Over Time
    ImGui::TextDisabled("NET WORTH ROLLING BALANCE TREND OVER TIME");
    ImGui::Spacing();
    
    std::vector<Transaction> sorted_txs = state.transactions;
    std::sort(sorted_txs.begin(), sorted_txs.end(), [](const Transaction& a, const Transaction& b) {
    ...
    for (const auto& acc : state.accounts) {
        initial_sum += acc.initial_balance;
    }
    
    std::vector<double> tx_indices;
    std::vector<double> cum_balances;
    
    tx_indices.push_back(0.0);
    cum_balances.push_back(initial_sum);
    
    double current_cum = initial_sum;
    for (size_t i = 0; i < sorted_txs.size(); ++i) {
        const auto& tx = sorted_txs[i];
        if (tx.type == "Income") {
            current_cum += tx.amount;
        } else if (tx.type == "Expense") {
            current_cum -= tx.amount;
        }
        tx_indices.push_back((double)(i + 1));
        cum_balances.push_back(current_cum);
    }
    
    if (ImPlot::BeginPlot("Balance Trend Line", ImVec2(-1, 230))) {
        ImPlot::SetupAxes("Transaction Ledger Step", "Total Balance (BDT)", ImPlotAxisFlags_AutoFit, ImPlotAxisFlags_AutoFit);
        ImPlot::SetNextLineStyle(ImVec4(0.41f, 0.34f, 0.74f, 1.00f), 2.5f);
        ImPlot::PlotLine("Cumulative Net Worth", tx_indices.data(), cum_balances.data(), (int)tx_indices.size());
        ImPlot::EndPlot();
    }
}
